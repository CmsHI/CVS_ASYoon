...
void MultiTrackValidator::beginRun(Run const&, EventSetup const& setup) {
  ...
  for (unsigned int ww=0;ww<associators.size();ww++){
	...
	h_assocSharedHit.push_back(dbe_->book1D("assocSharedHit","number of shared hits",20,0,20));
      ...
      // Andre
      n_pt_vs_eta_sim.push_back( dbe_->book2D("num_simul_pT_vs_eta","pT vs eta",nint,min,max,nintpT,minpT,maxpT) );
      n_pt_vs_eta_sim2rec.push_back( dbe_->book2D("num_assoc(simToReco)_pT_vs_eta","pT vs eta",nint,min,max,nintpT,minpT,maxpT) );
      n_pt_vs_eta_rec.push_back( dbe_->book2D("num_reco_pT_vs_eta","pT vs eta",nint,min,max,nintpT,minpT,maxpT) );
      n_pt_vs_eta_rec2sim.push_back( dbe_->book2D("num_assoc(recoToSim)_pT_vs_eta","pT vs eta",nint,min,max,nintpT,minpT,maxpT) );
      ...
      //Andre -> BinLogY
      //BinLogX(n_pt_vs_eta_sim[j]->getTH2F());
  ...
  for (unsigned int ww=0;ww<associators.size();ww++){
	...
	for (unsigned int f=0; f<etaintervals[w].size()-1; f++){
	   if (getEta(momentumTP.eta())>etaintervals[w][f]&&
	      ...
	      for (unsigned int g=0; g<pTintervals[w].size()-1; g++){
		 if (getPt(sqrt(momentumTP.perp2()))>pTintervals[w][g]&&
		     getPt(sqrt(momentumTP.perp2()))<pTintervals[w][g+1]) {
		    totSIM_pt_eta[w][f][g]++;
		    if(rt.size()!=0){
		       totASS_pt_eta[w][f][g]++;
		    }
	...
	// Andre 
	   ...
	   if (getEta(track->momentum().eta())>etaintervals[w][f]&&
	       getEta(track->momentum().eta())<etaintervals[w][f+1]) {
              ...
              for (unsigned int g=0; g<pTintervals[w].size()-1; g++){
		 if (getPt(sqrt(track->momentum().perp2()))>pTintervals[w][g]&&
		     getPt(sqrt(track->momentum().perp2()))<pTintervals[w][g+1]) {
		    totREC_pt_eta[w][f][g]++;
                    if (tp.size()!=0) {
                       totASS2_pt_eta[w][f][g]++;
                    }
                 }
              } // pt 
           } // eta 
...
// 	  double phiRec    = rParameters[2];
	  ...
	  // Andre:  eta vs pt
	  //n_pt_vs_eta_sim[w]->Fill(getEta(tpr.eta()),tpr.pt());
...
//       doProfileX(phires_vs_phi[w],h_phiresmean_vs_phi[w]);
      ...
      // Andre
      fillPlotFromVector2D(n_pt_vs_eta_sim[w],totSIM_pt_eta[w]);
      fillPlotFromVector2D(n_pt_vs_eta_sim2rec[w],totASS_pt_eta[w]);
      fillPlotFromVector2D(n_pt_vs_eta_rec[w],totREC_pt_eta[w]);
      fillPlotFromVector2D(n_pt_vs_eta_rec2sim[w],totASS2_pt_eta[w]);
      w++;
    }
  }

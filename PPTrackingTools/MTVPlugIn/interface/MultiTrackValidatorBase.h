...
class MultiTrackValidatorBase {
 public:
  ...
  // Andre
  void fillPlotFromVector2D(MonitorElement* h, std::vector< std::vector<int> >& vec) {
     for (unsigned int f=0; f<vec.size(); f++){
	for (unsigned int g=0; g<vec[f].size(); g++){
	   h->setBinContent(f+1,g+1,vec[f][g]);
	   //h->setBinContent(f+1,g+1,1); 
	}	
     }
  }
  ...
  void setUpVectors() {
    ...
    //Andre
    std::vector< std::vector<int> >   totSIMv_pt_eta, totASSv_pt_eta;
    std::vector< std::vector<int> >   totRECv_pt_eta, totASS2v_pt_eta;
    ...
    //Andre
    for (int j=1;j<nint+1;j++){ // eta
       std::vector<int> v;
       for (int k=1;k<nintpT+1;k++){ // pt
	  v.push_back(0);
       }
       totSIMv_pt_eta.push_back(v);
       totASSv_pt_eta.push_back(v);
       totRECv_pt_eta.push_back(v);
       totASS2v_pt_eta.push_back(v);
    }
    totSIM_pt_eta.push_back(totSIMv_pt_eta);
    totASS_pt_eta.push_back(totASSv_pt_eta);
    totREC_pt_eta.push_back(totRECv_pt_eta);
    totASS2_pt_eta.push_back(totASS2v_pt_eta);
 ...
 protected:
  ...
  // Andre
  std::vector<MonitorElement*> n_pt_vs_eta_sim;
  std::vector<MonitorElement*> n_pt_vs_eta_rec;
  std::vector<MonitorElement*> n_pt_vs_eta_sim2rec;
  std::vector<MonitorElement*> n_pt_vs_eta_rec2sim;
  ...
  // Andre
  std::vector< std::vector< std::vector<int> > > totSIM_pt_eta, totASS_pt_eta;
  std::vector< std::vector< std::vector<int> > > totREC_pt_eta, totASS2_pt_eta;

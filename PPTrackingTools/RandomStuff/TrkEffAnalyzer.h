...
class TrkEffAnalyzer : public edm::EDAnalyzer {
   ...
   private:
      ...
      SimTrack_t setSimTrack(TrackingParticle&, const reco::Track&, size_t, float, int, int);
      RecTrack_t setRecTrack(reco::Track&, const TrackingParticle&, size_t, float, int, int);
      bool testVertex(reco::Track&, double&, double&);
      std::pair<bool,bool> isAccepted(TrackingParticle&);
      int getLayerId(const PSimHit&);
      int getEvtMultiplicity(edm::Handle<TrackingParticleCollection>&);
      int getEvtMultiplicity(edm::Handle<edm::View<reco::Track> >&);
      std::pair<int,float> getLocalTrkDensity(edm::Handle<TrackingParticleCollection>&, TrackingParticle&, float&);
      std::pair<int,float> getLocalTrkDensity(edm::Handle<edm::View<reco::Track> >&, reco::Track&, float&);
      // ----------member data ---------------------------

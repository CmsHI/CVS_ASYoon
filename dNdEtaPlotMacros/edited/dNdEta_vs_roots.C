...
void dNdeta_vs_roots() {
  ...
  dummy->SetYTitle("dN_{ch}/d#eta_{|#eta|<0.5}");
  /*
...
//  TF1 *fitUA5 = new TF1("fitUA5","2.26-2*0.207*log(x)+0.0215*4*log(x)*log(x)",40,8000);
  ...
  /*
  TF1 *fitUA52 = new TF1("fitUA52","0.8+0.056*exp(sqrt(log(x^2)))",11,100000);
  fitUA52->SetLineColor(kBlue);
  if(bw)fitUA52->SetLineColor(kBlack);
  fitUA52->SetLineWidth(2);
  fitUA52->SetLineStyle(26);
  fitUA52->Draw("same");
  */

  //TF1 *fitUA5inel = new TF1("fitUA5inel","1.54-2*0.096*log(x)+0.0155*4*log(x)*log(x)",40,8000); 
  //TF1 *fitUA5inel = new TF1("fitUA5inel","0.691197+0.0612568*exp(sqrt(log(x^2)))",11,100000);
...
//  Double_t y7[2] =  {  2.36, 3.80 };  // from Yen-Jie PAS (PYTHIA)
  ...
  */
  ...
  TLegend* leg = new TLegend(0.50,0.885 - 0.045*5,0.80,0.885);
    ...
    leg2->AddEntry(fitUA5inel,"0.691 + 0.0613 exp(#sqrt{ln(s)})","l");

...
// * CorrectTypeTwoNSaveV2 uses a fit function to correct for mom. res. instead of on the fly smearing 
//   (e.g. doSemaing() is commented out)
// 
...
void CorrectTypeTwoNSaveV2(const char *cDir="../root_files/",
			   ...
			   )
{
   ...
   dum2 = GetDummyHist(0.5,200,(2E-11)*(scaleF),(2E+8)*(scaleF),"p_{T}","d^{3}N/d^{3}p",true);
   dum2->Draw();
   ...
   resCorrF = new TF1("CorrectionFunction","pol4",0.1,200);
   resCorrF->SetParameters(1.00418,-6.5107e-05,4.36783e-06,-4.29539e-08,1.50819e-10); // parameters are obtained separately
   ...
   dum7 = GetDummyHist(0.5,200,(2E-9)*(scaleF),(2E+8)*(scaleF),"p_{T}","d^{3}N/d^{3}p",true);
   dum7->Draw();
...
void doSmearing(){
   ...
   dum3 = GetDummyHist(0.5,200,(2)*(1),(1E+9)*(1),"p_{T}^{rec} or p_{T}^{sim}","dN/dp_{T}",true);
   ...
   dum5 = GetDummyHist(0.0,200,1E-7,10,"p_{T}^{rec}","Prob",false);

...
// * on-the-fly smearing part is omitted in place of fit function based correction. 
//   (i.e. doSmearing() is commented out) This is due to the fact that momentum resolution 
//   with variable binning shows a osciallatory behavior. 
// * on-the-fly smearing can be used for a fixed size pT bin!
// 
...
TH1D *hRes1D_sim=0;
TH2D *hRes2D = 0;
TH2D *hRes2DWeight = 0; // to be smeared
TH3F *hRes3D = 0;

TH1D *hLev1Spec=0, *hLev1SpecR=0;
TH1D *hLev2Spec=0, *hLev2SpecR=0;

TH1D *hLev1Spec_dndpt=0;
TH1D *hLev2Spec_dndpt=0;
...
double scaleF=1;
double maxpT = 360; // max pT in the histograms
...
double integralVariableBin(TH1D* hist);
TH1D *DivideByBinW(TH1D* hist);
TH1D *MultiplyByBinW(TH1D* hist);
TH1D *weigthWithFunc(TH1D *hist,TF1 *cf);
//void checkEtaRange(float iEta, float fEta, int EtaMin, int EtaMax);
//------------------------------------------------------------------------------ 

void HiCorrectTypeTwoNSave(const char *cDir="../root_files/",   // root file for mom. res.
			 const char *cDirSec="../rootOutput/", // output root file to be updated
			 ...
			 double scaleFin=1,
			 bool rebOnly=false, int rebinF=1,
			 ...
			 )
{
   ...
   sprintf(inFile,"%s%s.root",cDirSec,cFile);   
   ...
   hfitF = FillFromFunc(hdNdpT,fitF); //this is histogram filled from the fit function
   //hfitF = (TH1D*) MultiplyByBinW(hfitF);
   hfitF->Draw("histsame");
   ...
   dum2 = GetDummyHist(0.5,maxpT,(2E-11)*(scaleF),(2E+8)*(scaleF),"p_{T}","d^{3}N/d^{3}p",true);
   dum2->Draw();
   
   obtainFitInv(xmin,xmax,2.*feta); //now obtain invariant yield analytically

   hfitF_inv = FillFromFunc(hdNdpT,fitF_inv); // fill from function!   
   //hfitF_inv = (TH1D*) MultiplyByBinW(hfitF_inv);
   ...
   if(!rebOnly){
	 ...
	 hRes3D->GetXaxis()->GetLast() : hRes3D->GetXaxis()->FindBin(feta-small);
      ...
      cout<<"eta min bin "<<eta_min_bin<<"eta max bin "<<eta_max_bin<<endl;

      hRes3D->GetXaxis()->SetRange(eta_min_bin,eta_max_bin); 
      hRes2D = (TH2D*) hRes3D->Project3D("zy");
      hRes2DWeight= (TH2D*) hRes2D->Clone("hRes2DWeight");
   ...
   can6->cd(), can6->SetLogz();
   if(!rebOnly) hRes2D->Draw("COLZ");
   ...
   resCorrF = new TF1("CorrectionFunction","[0]+[1]*x+[2]*x^2+[3]*(x+1)^(-1)",0.1,maxpT);
   resCorrF->SetParameters(1.00312e+00,1.18975e-04,2.18865e-07,1.01054e-02); // parameters are obtained from testMomRes.C

   hLev2Spec = (TH1D*) weigthWithFunc(hfitF,resCorrF);  // weighted with correction factor for mom resolution 
   hLev2Spec = (TH1D*) makeItInvHist(hLev2Spec,2.*feta);

   hLev1SpecR = (TH1D*) hLev1Spec->Rebin(rebinF,"hLev1SpecR"), hLev1SpecR->Scale((double)(1./rebinF));
   hLev2SpecR = (TH1D*) hLev2Spec->Rebin(rebinF,"hLev2SpecR"), hLev2SpecR->Scale((double)(1./rebinF));
   ...
   dum4 = GetDummyHist(0.5,maxpT,0.7,2.8,"p_{T}^{rec}","Correction factor",false);
   dum4->Draw("");

   TLine *line = new TLine(0.5,1,maxpT,1);
   line->SetLineStyle(2);
   line->SetLineWidth(2);
   line->Draw();
   ...
   TH1D *hCorrDivide=0;
   hCorrDivide = (TH1D*) hCorrLev2->Clone("hCorrDivide");
   hCorrDivide->Divide(hCorrLev1);
   //hCorrDivide->SetMarkerStyle(20), hCorrDivide->Draw("pzsame");
   ...
   hInvCorrOneR = (TH1D*) hInvCorrOne->Rebin(rebinF,"hInvCorrOneR"), hInvCorrOneR->Scale((double)(1./rebinF));
   ...
   dum7 = GetDummyHist(0.5,maxpT,(2E-9)*(scaleF),(2E+8)*(scaleF),"p_{T}","d^{3}N/d^{3}p",true);
   dum7->Draw();
...
void doSmearing(){
   ...
   dum3 = GetDummyHist(0.5,maxpT,(2)*(1),(1E+9)*(1),"p_{T}^{rec} or p_{T}^{sim}","dN/dp_{T}",true);
   dum3->SetTitle("raw dn/dpt from resolution histogram"), dum3->SetName("dum3");
   dum3->Draw();

   // dn/dpt with rec pt
   hRes1D = (TH1D*) hRes2DWeight->ProjectionY("hRes1D",hRes2D->GetXaxis()->GetFirst(),hRes2D->GetXaxis()->GetLast(),"e");
   hRes1D->SetMarkerSize(1), hRes1D->Draw("histsame"); 

   hRes1D_sim = (TH1D*) hRes2DWeight->ProjectionX("hRes1D_sim",hRes2D->GetYaxis()->GetFirst(),hRes2D->GetYaxis()->GetLast(),"e");
   hRes1D_sim->SetMarkerStyle(24), hRes1D_sim->SetMarkerSize(1), hRes1D_sim->SetMarkerColor(2);
   hRes1D_sim->Draw("pzsame"); 

   can5 = new TCanvas("can5","can5",550,410);
   can5->cd(), can5->SetLogy();

   dum5 = GetDummyHist(0.0,maxpT,1E-7,10,"p_{T}^{rec}","Prob",false);
   dum5->SetTitle("rec p_{T} distribution at a given sim p_{T}");
   dum5->SetName("dum5");
   ...
   for(int i=0;i<nbin;i++){
      ...
      /*
      if(nbin_dndpt!= hRes2DWeight->GetNbinsY()){
	 Error("[doSmearing]","");
         return;
      }
      */

      hRes1D_temp = (TH1D*) hRes2DWeight->ProjectionY("",i+1,i+1,"e");
      //TH1D *hRes1D_temp  = (TH1D*) hRes2DWeight->ProjectionY("hRes1D_temp",i+1,i+1,"e"); 
      hRes1D_temp->SetName(Form("hRes1D_temp_%d",i+1));
      hRes1D_temp = (TH1D*) DivideByBinW(hRes1D_temp);

      //- normalize the distribution  
      //double area = hRes1D_temp->Integral();
      double area = integralVariableBin(hRes1D_temp);
      //cout<<"Area = "<<area<<endl;
      ...
      if(((i+1)%10)==0){
	hRes1D_temp->SetLineColor(color[(int)(i/10)]);
	hRes1D_temp->Draw("histsame");
	cout<<"first bin pt : "<<hRes1D_temp->GetXaxis()->GetBinCenter(firstB)<<" at bin : "<<firstB
	    <<" last bin pt : "<<hRes1D_temp->GetXaxis()->GetBinCenter(lastB)<<" at bin : "<<lastB<<endl;
      }
	 ...
	 int rpt_bin = hRes2DWeight->GetYaxis()->FindBin(ptr); // should be same as j!                                                                         
	 int spt_bin = i+1;
	 ...
	 hRes2DWeight->SetBinContent(spt_bin,rpt_bin,smeared);
	 hRes2DWeight->SetBinError(spt_bin,rpt_bin,0);
      }
      //delete hRes1D_temp;
   }
...
TH3F *makeItdNdpT(TH3F *h3Dhist){
  // This takess 3D histogram and for each entry, scale it with dbin      
  // so that when projected, the projected one is dN/dpT nod dN
...
void fitdNdpT(float ix, float fx){
   ...
   dum1 = GetDummyHist(0.5,maxpT,(2E-6)*(scaleF),(1E+9)*(scaleF),"p_{T}","dN/dp_{T}",true);
   dum1->SetTitle("raw dn/dpt fitted"), dum1->SetName("dum1");
   dum1->Draw();
   
   fitFormula = "[0]*([5]+(x/[1])+(pow(x,[2])/[3]))^[4]";
   //fitFormula = "([0]*exp([1]*x))";
   ...
   //fitF->SetParameters((2.27653e+08)*(scaleF),3.43178e+00,1.57503e+00,3.17222e+00,-3.85370e+00,6.86663e-01);
   //fitF->SetParameters((4.42247e+08)*(scaleF),2.66628e+00,5.02453e-01,1.18727e+00,-6.15058e+00,-2.65865e-01);
   //fitF->SetParameters((7.84642e+07)*(scaleF),1.17154e+00,9.91281e-01,5.67177e-01,-4.06651e+00,-4.58692e+01);
   //fitF->SetParameters((1.23650e+02)*(scaleF),-9.07859e-02);
   //fitF->SetParameters((2.74006e+00)*(scaleF),-4.32193e-02);
...
TH1D *weigthWithFunc(TH1D *hist,TF1 *cf){
  TH1D *hfitFW = (TH1D*) hist->Clone("hfitFW");
  hfitFW->Reset();
  for(int i=0;i<hfitFW->GetNbinsX();i++){
    double content = hist->GetBinContent(i+1);
    double err = hist->GetBinError(i+1);
    double pt = hist->GetBinCenter(i+1);
    double weight = cf->Eval(pt);
    hfitFW->SetBinContent(i+1,content*weight);
    hfitFW->SetBinError(i+1,err*weight);
  }
  return hfitFW;
}
...
double integralVariableBin(TH1D* hist){
  ...
  for(int i=0;i<hist->GetNbinsX();i++){
    double dbin = hist->GetXaxis()->GetBinWidth(i+1);
    double content = hist->GetBinContent(i+1);
    //a = a + content*dbin;
    a = a + content;
  }

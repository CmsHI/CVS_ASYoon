...
namespace {
  ...
  template <class T> struct GetRawEtRef {inline double getValue (const T& a) {return a->correctedJet("Uncorrected").et();}};
  ...
  template <class T>
   ...
   template <class T>
     inline void sortByRawEtRef (std::vector <T>* container) {
     sortGreater <T, GetRawEtRef<T> > (container);
   }

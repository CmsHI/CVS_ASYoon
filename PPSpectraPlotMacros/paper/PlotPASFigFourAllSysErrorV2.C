...
double returnCombinedSysError(double pt);
...
double returnCombinedSysError(double pt){
  ...
  TF1 *fEvt = new TF1("fEvt","0.035",0,200);
  TF1 *fVtx = new TF1("fVtx","0.012",0,200);
  TF1 *fAcc = new TF1("fAcc","0.015",0,200);
  TF1 *fEff = new TF1("fEff","0.022",0,200);
  TF1 *fOcc = new TF1("fOcc","0.001+(x>20)*(x-20.)*0.015/70.*(x<135) + (x>134)*(0.0273/(1.0+exp(-0.1*(x-110.))))",0,200);
  TF1 *fFak = new TF1("fFak","0.003+x*0.027/150.*(x<135) + (x>134)*(0.029/(1.0+exp(-0.1*(x-118.))))",0,200);
  TF1 *fSec = new TF1("fSec","0.01",0,200);
  TF1 *fMom = new TF1("fMom","0.003+0.024/(1.0+exp(-0.2*(x-40.)))",0,200);
  TF1 *fTrg = new TF1("fTrg","0.033/(1.0+exp(-0.4*(x-32.))) + 0.007/(1.0+exp(-0.4*(x-58.)))",0,200);

  double err_sum = 0;
  err_sum += pow(fEvt->Eval(pt),2);
  err_sum += pow(fVtx->Eval(pt),2);
  err_sum += pow(fAcc->Eval(pt),2);
  err_sum += pow(fEff->Eval(pt),2);
  err_sum += pow(fOcc->Eval(pt),2);
  err_sum += pow(fFak->Eval(pt),2);
  err_sum += pow(fSec->Eval(pt),2);
  err_sum += pow(fMom->Eval(pt),2);
  err_sum += pow(fTrg->Eval(pt),2);
  err_sum = TMath::Sqrt(err_sum);

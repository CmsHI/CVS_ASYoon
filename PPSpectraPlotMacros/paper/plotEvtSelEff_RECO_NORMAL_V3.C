...
void plotEvtSelEff_RECO_NORMAL_V3() {
  ...
  //TFile *f = new TFile("../root_files/TrkHistGEN_aug29_qcdMB10M357EitherHF.root");
  ...
  if(includeCorr){
     ...
     TH1F *dum2 = new TH1F("dum2",";Charged particle multiplicity;#varepsilon_{NSD}^{selected} (f_{SD}^{selected})",
			   100,0,25);
     dum1->SetMaximum(0.045);
  }else{
     TH1F *dum1 = new TH1F("dum1",";Charged particle multiplicity;Fraction of events",100,-2,100);
     TH1F *dum2 = new TH1F("dum2",";Charged particle multiplicity;#varepsilon_{NSD}^{selected}  or  f_{SD}^{selected}",
			   100,0,55);
  }
  ...
  //plotting
  th1Style1(hNSD,1,25,1.0,1,1.5,1,1);
  th1Style1(hNSDtrkvtx,1,24,1.0,1,1.5,1,1);
  th1Style1(hNSDvtx,1,20,1.0,1,1.5,1,1);
  if(includeCorr) {
  ...
  if(includeCorr) {
     leg1->AddEntry(hNSDvtx_CORR,"pixel vertex (corr.)","pl");
     leg1->AddEntry(hNSDtrkvtx_CORR,"track vertex (corr.)","pl");
  }
  leg1->Draw();
  ...
  th1Style1(gSDvtx,2,22,1.0,1,2,9,3);
  th1Style1(gSDtrkvtx,4,26,1.0,1,2,2,3);

  th1Style1(gNSDvtx,1,20,1.0,1,2,1,1);
  th1Style1(gNSDtrkvtx,1,24,1.0,1,2,1,1);

  if(includeCorr){
     th1Style1(gNSDvtx_CORR,2,20,1.0,2,2,1,1);
     th1Style1(gNSDtrkvtx_CORR,4,24,1.0,4,2,1,1);
  }



  TLatex *tex2 = new TLatex(0.46,0.69,"PYTHIA 7 TeV");
  ...
  double y1 = 0.48;
  double y2 = 0.66;
  if(includeCorr) TLegend *leg2 = new TLegend(0.42,0.39,0.80,0.63); 
  else TLegend *leg2 = new TLegend(0.45,y1,0.83,y2);
  leg2->SetMargin(0.3);
  ...
  leg2->SetHeader("NSD selection efficiency");
  leg2->AddEntry(gNSDvtx,"pixel vertex (NSD)","pl");
  leg2->AddEntry(gNSDtrkvtx,"track vertex (NSD)","pl");
  leg2->Draw();
  
  if(includeCorr) TLegend *leg3 = new TLegend(0.42,0.29,0.80,0.63);
  else TLegend *leg3 = new TLegend(0.45,y1-0.21,0.83,y2-0.21);
  leg3->SetMargin(0.3);
  ...
  leg3->SetMargin(0.25);
  leg3->SetHeader("Selected event SD fraction");
  leg3->AddEntry(gSDvtx,"pixel vertex (SD)","l");
  leg3->AddEntry(gSDtrkvtx,"track vertex (SD)","l");
  if(includeCorr) {
     leg3->AddEntry(gNSDvtx_CORR,"pixel vertex (corr.)","pl");
     leg3->AddEntry(gNSDtrkvtx_CORR,"4 track vertex (corr.)","pl");
  }
  leg3->Draw();
